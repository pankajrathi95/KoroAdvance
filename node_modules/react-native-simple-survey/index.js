...
export class SimpleSurvey extends Component {
    ...
    constructor(props) {
        super(props);
        this.state = { currentQuestionIndex: 0, answers: [], initialQuestionIndex: 0 };
    ...
    renderSelectionGroup(currentIndex) {
    ...
    render() {
        const { survey } = this.props;
        let currentQuestion = this.state.currentQuestionIndex;
        let {initialQuestionIndex} = this.state;
        //let initialQuestionIndex = this.state.initialQuestionIndex;
        const followUpQuestion = survey[currentQuestion].followUpQuestion;
        const firstQuestion = survey[currentQuestion].firstQuestion;
        if (firstQuestion)
        {
            this.openAddBoardModal();
        }

        if (followUpQuestion != undefined && followUpQuestion && this.state.answers[this.state.initialQuestionIndex].value.value == "0") {
            currentQuestion++;
            let {currentQuestionIndex} = this.state;
            currentQuestionIndex++;
            this.setState({currentQuestionIndex});
        ...
        switch (survey[currentQuestion].questionType) {
            case 'SelectionGroup': return this.renderSelectionGroup(currentQuestion);
            case 'MultipleSelectionGroup': return this.renderMultipleSelectionGroup();
            case 'TextInput': return this.renderTextInputElement();
            case 'NumericInput': return this.renderNumeric(currentQuestion);
    ...
    async openAddBoardModal(){
        let {currentQuestionIndex} = this.state;
           // () => this.setState({initialQuestionIndex : currentQuestionIndex});
        await this.setState({initialQuestionIndex : currentQuestionIndex});
        console.log(this.state.initialQuestionIndex);
    }
}
